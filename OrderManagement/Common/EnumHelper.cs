...
namespace OrderManagement.Common
{
    public class EnumHelper
    {
        ...
        //fixed Warning 1: Dereference of a possibly null reference
        public static int? GetEnumValueFromDisplayName<TEnum>(string displayName) where TEnum : struct, Enum
        {
            foreach (var field in typeof(TEnum).GetFields())
            {
                ...
                if (attribute?.Name != null &&
                    attribute.Name.Equals(displayName, StringComparison.CurrentCultureIgnoreCase))
                {
                    var rawValue = field.GetValue(null);
                    return rawValue is int intValue ? intValue : null;
                }
            }
            return null;
        }
